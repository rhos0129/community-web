  ...
  return (
    <>
      ...
      <ListGroup className='w-100' as='ul' >
        {
          items.map((item, key) => (
            <ListGroup.Item className='border-start-0 border-end-0 rounded-0' action
              as='li' key={key}>
              <h5 className='w-75 fw-bolder text-truncate fs-6'>
                <a className='link-bluegray-800 link-underline-opacity-0 link-opacity-50-hover'
                  href={item.href}>
                  {item.title}
                </a>
              </h5>
